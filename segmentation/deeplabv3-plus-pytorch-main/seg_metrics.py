...
#################################################################
#  标签图像文件夹
LabelPath = r"C:\Users\zts\Desktop\me\label\val-label-8bit.tif"
#  预测图像文件夹
PredictPath = r"C:\Users\zts\Desktop\me\36\ground-val.tif"
#  类别数目(包括背景)
classNum =3
#################################################################
...
def color_dict(labelFolder, classNum):
    ...
    for i in range(len(ImageNameList)):
        ...
        img_new = img[:,:,0] + img[:,:,1] * 1000 + img[:,:,2]* 1000000 
...
def ConfusionMatrix(numClass, imgPredict, Label):  
    ...
    print(np.unique(Label[mask]))
    print(np.unique(imgPredict[mask]))  
    print(np.unique(label ))  
    print(np.unique( numClass**2))
...
for i in range(label_num):
    ...
    #Predict = readTif(PredictPath + "/" + PredictList[i])
    #unique = np.unique(predict_all )
    #print(unique)
    #Predict = GdalData2OpencvData(Predict)
